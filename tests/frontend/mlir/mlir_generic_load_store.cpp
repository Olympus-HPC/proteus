...
// CHECK: %[[SIDX:[0-9]+]] = memref.load %{{.*}}[%{{.*}}] : memref<1xindex>
// CHECK: memref.store %c42_i32, %{{.*}}[%[[SIDX]]] : memref<4xi32>
// CHECK: %[[LIDX:[0-9]+]] = memref.load %{{.*}}[%{{.*}}] : memref<1xindex>
// CHECK: %{{.*}} = memref.load %{{.*}}[%[[LIDX]]] : memref<4xi32>
// clang-format on
