// clang-format off
// RUN: rm -rf "%t.$$.proteus"
// RUN: PROTEUS_CACHE_DIR="%t.$$.proteus" PROTEUS_TRACE_OUTPUT="cache-stats" %build/mlir_gpu_operators.%ext | %FILECHECK %s --check-prefixes=CHECK,CHECK-FIRST
// Second run uses the object cache.
// RUN: PROTEUS_CACHE_DIR="%t.$$.proteus" PROTEUS_TRACE_OUTPUT="cache-stats" %build/mlir_gpu_operators.%ext | %FILECHECK %s --check-prefixes=CHECK,CHECK-SECOND
// RUN: rm -rf "%t.$$.proteus"
// clang-format on
...
int main() {
  ...
  F.beginFunction();
  {
    ...
    *Arg6 = *Arg7 = *Arg8 = *Arg9 = 5;
    ...
    Var<double &> Ref14 = *Arg14;
    Ref14 = 42.0;

    // Check that Var<T[]> operator[] returns Var<T&>.
    Var<double &> Ref15 = Arg15[0];
    Ref15 = 43.0;
    ...
    auto Ref17 = *Arg17;
    auto Addr17 = Ref17.getAddress();
    auto RefAddr17 = *Addr17;
    RefAddr17 = 44.0;
  ...
  gpuErrCheck(gpuMallocManaged(&R17, sizeof(double)));
