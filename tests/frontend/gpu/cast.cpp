// clang-format off
// RUN: rm -rf "%t.$$.proteus"
// RUN: PROTEUS_CACHE_DIR="%t.$$.proteus" %build/cast.%ext | %FILECHECK %s --check-prefixes=CHECK,CHECK-FIRST
// Second run uses the object cache.
// RUN: PROTEUS_CACHE_DIR="%t.$$.proteus" %build/cast.%ext | %FILECHECK %s --check-prefixes=CHECK,CHECK-SECOND
// RUN: rm -rf "%t.$$.proteus"
// clang-format on
...
int main() {
  ...
  gpuErrCheck(gpuMallocManaged(&RD1, sizeof(double)));
  gpuErrCheck(gpuMallocManaged(&RI1, sizeof(int)));
  ...
  gpuErrCheck(gpuFree(RD0));
  gpuErrCheck(gpuFree(RI0));
  gpuErrCheck(gpuFree(RF0));
  gpuErrCheck(gpuFree(RD1));
  gpuErrCheck(gpuFree(RI1));
  gpuErrCheck(gpuFree(RF1));
  gpuErrCheck(gpuFree(RI2));
