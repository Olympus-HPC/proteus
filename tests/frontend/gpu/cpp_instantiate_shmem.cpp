// clang-format off
// RUN: rm -rf "%t.$$.proteus"
// RUN: PROTEUS_CACHE_DIR="%t.$$.proteus" %build/cpp_instantiate_shmem.%ext | %FILECHECK %s
// RUN: rm -rf "%t.$$.proteus"
// clang-format on

#if !PROTEUS_ENABLE_CUDA && !PROTEUS_ENABLE_HIP
#error                                                                         \
    "cpp_instantiate_shmem requires PROTEUS_ENABLE_CUDA or PROTEUS_ENABLE_HIP"
#endif
...
constexpr const char *Target = "hip";
#define INCLUDE "#include <hip/hip_runtime.h>"
#elif PROTEUS_ENABLE_CUDA
constexpr const char *Target = "cuda";
