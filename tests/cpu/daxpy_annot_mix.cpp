// RUN: rm -rf .proteus
// RUN: PROTEUS_TRACE_OUTPUT=1 ./daxpy_annot_long | %FILECHECK %s
// --check-prefixes=CHECK RUN: rm -rf .proteus
...
__attribute__((annotate("jit", 1, "arg=4"))) void myDaxpy(double A, double *X,
                                                          double *Y, size_t N) {
