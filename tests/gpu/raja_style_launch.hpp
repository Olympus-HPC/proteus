...
template<typename T>
__host__ __device__ auto thread_privatize(const T& item) -> Privatizer<T>
{
...
void
globalWrapper(Lambda lam, size_t N) {
  auto privatizer = thread_privatize(lam);
...
void
forall(size_t N, Lambda&& lam, Args&&... args) {
  ...
  void *AArgs[] = {&lam, &N};
  cudaLaunchKernel(func, dim3(GridSize), dim3(256), AArgs,
                  0, 0);
#else
