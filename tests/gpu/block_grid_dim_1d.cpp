// clang-format off
// RUN: rm -rf "%t.$$.proteus"
// RUN: PROTEUS_CACHE_DIR="%t.$$.proteus" PROTEUS_SPECIALIZE_DIMS_RANGE=0 PROTEUS_SPECIALIZE_DIMS_ASSUME=1  PROTEUS_TRACE_OUTPUT=1 %build/block_grid_dim_1d.%ext | %FILECHECK %s --check-prefixes=CHECK,CHECK-FIRST
// Second run uses the object cache.
// RUN: PROTEUS_CACHE_DIR="%t.$$.proteus" PROTEUS_SPECIALIZE_DIMS_RANGE=0 %build/block_grid_dim_1d.%ext | %FILECHECK %s --check-prefixes=CHECK,CHECK-SECOND
// RUN: rm -rf "%t.$$.proteus"
// RUN: PROTEUS_CACHE_DIR="%t.$$.proteus" PROTEUS_SPECIALIZE_DIMS=0 PROTEUS_SPECIALIZE_DIMS_RANGE=1 PROTEUS_SPECIALIZE_DIMS_ASSUME=0 PROTEUS_TRACE_OUTPUT=1 %build/block_grid_dim_1d.%ext | %FILECHECK %s --check-prefixes=RANGE
// RUN: rm -rf "%t.$$.proteus"
// clang-format on
...
// RANGE: [DimSpec] Range {{llvm.amdgcn.workitem.id.x|llvm.nvvm.read.ptx.sreg.tid.x}} [0,64)
// RANGE: [DimSpec] Range {{llvm.amdgcn.workgroup.id.x|llvm.nvvm.read.ptx.sreg.ctaid.x}} [0,1)
