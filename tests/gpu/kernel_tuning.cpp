// clang-format off
// RUN: rm -rf "%t.$$.proteus"
// RUN: PROTEUS_TUNED_KERNELS=%S/tuned_config.json PROTEUS_TRACE_OUTPUT="specialization;cache-stats" %build/kernel_tuning.%ext | %FILECHECK %s --check-prefixes=CHECK,CHECK-%device_lang
// RUN: rm -rf "%t.$$.proteus"
// clang-format on
...
// CHECK: [KernelConfig] ID:bar CG:RTC SA:1 LB:1 SD:1 SDR:0 OL:3 CGL:2 TMT:4 BPSM:4
...
// CHECK-HIP: [KernelConfig] ID:baz CG:{{RTC|Serial|Parallel}} SA:1 LB:1 SD:1 SDR:0 OL:3 CGL:3 TMT:-1 BPSM:0
// CHECK: [LaunchBoundSpec] MaxThreads 1 MinBlocksPerSM 0
