// clang-format off
// RUN: rm -rf "%t.$$.proteus"
// RUN: PROTEUS_TUNED_KERNELS=%S/tuned_config.json PROTEUS_TRACE_OUTPUT=1 %build/kernel_tuning.%ext | %FILECHECK %s --check-prefixes=CHECK
// RUN: rm -rf "%t.$$.proteus"
// clang-format on
...
// CHECK: [KernelConfig] ID:baz CG:RTC SA:1 LB:1 SD:1 SDA:0 OL:3 CGL:3
// CHECK: [LaunchBoundSpec] BlockSize 1
// CHECK: Hello from baz
// CHECK: JitCache hits 0 total 3
// CHECK: HashValue {{[0-9]+}} NumExecs 1 NumHits 0 FnName baz
// CHECK: HashValue {{[0-9]+}} NumExecs 1 NumHits 0 FnName foo
// CHECK: HashValue {{[0-9]+}} NumExecs 1 NumHits 0 FnName bar
