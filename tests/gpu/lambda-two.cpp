...
template <typename T, typename L> void registerRun(T &&LB, L &&LB2) {
  ...
  kernel<<<NumBlocks, BlockSize>>>(
    ...
    //Abstraction(
      PROTEUS_REGISTER_LAMBDA(LB2)//)
  );
  gpuErrCheck(gpuDeviceSynchronize());
}

auto declareLambda(int rc1, int rc2) {
    return [=, rc1 = proteus::jit_variable(rc1), rc2 = proteus::jit_variable(rc2)] __attribute__((annotate("jit"))) (size_t)
                        {
                          printInt(rc1);
                          printInt(rc2);
                        };
...
inline void launch(int C, int D) {
  registerRun([=, C = proteus::jit_variable(C)] __device__
              __attribute__((annotate("jit"))) (size_t) { printf("Integer = %d\n", C); },
              [=, D = proteus::jit_variable(D)] __device__
              __attribute__((annotate("jit"))) (size_t) { printf("Integer = %d\n", D); });
...
int main() {
  launch(1, 2);
  // registerRun(declareLambda(1,2), declareLambda(2, 3));
  // forall(10, declareLambda(1, 4));
  // forall(10, declareLambda(5, 6));
  registerRun(
    declareLambda(42, 24),
    declareLambda(65, 56)
  );
