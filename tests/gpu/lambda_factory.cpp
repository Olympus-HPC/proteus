...
// auto declareLambda(int RC1, int RC2, bool flag) {
//   if (flag)
//     return [=, C1 = proteus::jit_variable(RC1),
//                 C2 = proteus::jit_variable(RC2)] ()
//                          __attribute__((annotate("jit"))) { printInt(C1); printInt(C2); };
//   return [=, C = proteus::jit_variable(RC1)] ()
//                          __attribute__((annotate("jit"))) { printInt(C); };
// }
...
int main() {
  ...
  auto ZeroLambda = declareLambda(Zero);

  auto OneLambda = declareLambda(One);

  auto TwoLambda = declareLambda(Two);
