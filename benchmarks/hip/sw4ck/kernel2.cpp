...
__global__
#endif
    void
    kernel2(const int start0, const int N0, const int start1, const int N1,
            const int start2, const int N2, const int ifirst, const int ilast,
            const int jfirst, const int jlast, const int kfirst,
            const int klast, const float_sw4 a1, float_sw4 sgn,
  ...
  if ((i < N0) && (j < N1) && (k < N2)) {
    ...
    // pr-derivatives
    // 130 ops., tot=475
    r1 += c2 * ((2 * mu(i, j, k + 2) + la(i, j, k + 2)) * met(2, i, j, k + 2) *
                    met(1, i, j, k + 2) *
                ...
                ((2 * mu(i, j, k - 2) + la(i, j, k - 2)) * met(2, i, j, k - 2) *
                 ...
                 mu(i, j, k - 2) * met(4, i, j, k - 2) * met(1, i, j, k - 2) *
                     (c2 * (u(3, i + 2, j, k - 2) - u(3, i - 2, j, k - 2)) +
                      c1 * (u(3, i + 1, j, k - 2) - u(3, i - 1, j, k - 2))) *
                     istry)) +
          c1 * ((2 * mu(i, j, k + 1) + la(i, j, k + 1)) * met(2, i, j, k + 1) *
                    met(1, i, j, k + 1) *
    ...
    // qr derivatives
    // 82 ops, tot=687
    r1 += c2 * (mu(i, j, k + 2) * met(3, i, j, k + 2) * met(1, i, j, k + 2) *
