...
if [ "${CI_MACHINE}" == "tioga" ]; then
  ...
  echo "### $(date) START TESTING SYNC COMPILATION WITH PROTEUS CODEGEN THINLTO ###"
  PROTEUS_CODEGEN=thinlto ctest -T test --output-on-failure
  echo "### $(date) END TESTING SYNC COMPILATION WITH PROTEUS HIP CODEGEN THINLTO ###"
  ...
  echo "### $(date) START TESTING (BLOCKING) ASYNC COMPILATION WITH PROTEUS CODEGEN THINLTO ###"
  PROTEUS_ASYNC_COMPILATION=1 PROTEUS_ASYNC_TEST_BLOCKING=1 PROTEUS_CODEGEN=thinlto ctest -T test --output-on-failure
  echo "### $(date) END TESTING (BLOCKING) ASYNC COMPILATION WITH PROTEUS CODEGEN THINLTO ###"
