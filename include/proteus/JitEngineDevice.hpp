...
public:
  ...
  Module &getModule(JITKernelInfo &KernelInfo) {
    ...
    if (Config.PROTEUS_USE_LIGHTWEIGHT_KERNEL_CLONE) {
      KernelModule = std::move(std::make_unique<Module>("JitModule", BinModule.getContext()));
      KernelModule->setSourceFileName(BinModule.getSourceFileName());
      KernelModule->setDataLayout(BinModule.getDataLayout());
      KernelModule->setTargetTriple(BinModule.getTargetTriple());
      KernelModule->setModuleInlineAsm(BinModule.getModuleInlineAsm());
      KernelModule->IsNewDbgInfoFormat = BinModule.IsNewDbgInfoFormat;

      auto KernelFunction =
          BinInfo.getModule().getFunction(KernelInfo.getName());
