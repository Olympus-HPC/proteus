...
public:
  ...
  /// Wrap the Nth argument of \p F as an \c IRValue* owned by this builder.
  IRValue *getArg(llvm::Function *F, size_t Idx);
  ...
  IRValue *loadScalar(IRValue *Slot, IRType ValueTy);
  /// Store \p Val directly into \p Slot (scalar alloca).
  void storeScalar(IRValue *Slot, IRValue *Val);
  /// Load the pointer stored in \p Slot (pointer alloca).
  IRValue *loadAddress(IRValue *Slot, IRType AllocTy);
  /// Store \p Addr into \p Slot (pointer alloca).
  void storeAddress(IRValue *Slot, IRValue *Addr);
  /// Dereference the pointer stored in \p Slot, then load the pointee.
  IRValue *loadFromPointee(IRValue *Slot, IRType AllocTy, IRType ValueTy);
  /// Dereference the pointer stored in \p Slot, then store \p Val to it.
  void storeToPointee(IRValue *Slot, IRType AllocTy, IRValue *Val);

  // Alloca factory methods — return a VarAlloc aggregate.
  VarAlloc allocScalar(const std::string &Name, IRType ValueTy);
