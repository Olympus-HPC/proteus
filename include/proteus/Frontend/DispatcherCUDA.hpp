...
public:
  ...
  compile([[maybe_unused]] std::unique_ptr<LLVMContext> Ctx,
          std::unique_ptr<Module> Mod, HashT ModuleHash, bool DisableIROpt = false) override {
    ...
    std::unique_ptr<MemoryBuffer> ObjectModule = Jit.compileOnly(*ModOwner, DisableIROpt);
    if (!ObjectModule)
