...
public:
  CompilationTask(
      ...
      : Ctx(std::make_unique<LLVMContext>()), M(clone(Mod)),
        HashValue(HashValue), KernelName(KernelName), Suffix(Suffix),
        BlockDim(BlockDim), GridDim(GridDim), RCIndices(RCIndices),
        RCVec(RCVec), VarNameToDevPtr(VarNameToDevPtr),
