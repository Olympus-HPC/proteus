...
public:
  ...
  static DeviceError_t launchKernelFunction(KernelFunction_t KernelFunc,
                                            dim3 GridDim, dim3 BlockDim,
                                            void **KernelArgs,
                                            uint64_t ShmemSize,
                                            DeviceStream_t Stream) {
    TIMESCOPE(__FUNCTION__);
    return ImplT::launchKernelFunction(KernelFunc, GridDim, BlockDim,
...
private:
  ...
  static Function *reduceIR(Module &M, StringRef FnName, dim3 &BlockDim,
                            dim3 &GridDim,
                            const SmallVector<int32_t> &RCIndices,
...
Function *
JitEngineDevice<ImplT>::reduceIR(Module &M, StringRef FnName, dim3 &BlockDim,
                                 dim3 &GridDim,
                                 const SmallVector<int32_t> &RCIndices,
                                 RuntimeConstant *RC, int NumRuntimeConstants) {
...
JitEngineDevice<ImplT>::compileAndRun(
  ...
  // further. We can have a reduceIR in which we do not do any RC/Grid/Block
  ...
  auto *JitFunc = reduceIR(*JitModule, KernelName, BlockDim, GridDim, RCIndices,
  ...
  JitFunc->setName(KernelName + Suffix);

  if (Config.ENV_PROTEUS_SET_LAUNCH_BOUNDS)
    setLaunchBoundsForKernel(*JitModule, *JitFunc,
  ...
  if (verifyModule(*JitModule, &errs()))
    ...
    // internally runs it. For the rest of cases, that is CUDA or HIP with our
    // own codegen instead of RTC, run the target-specific optimization pipeline
    // to optimize the LLVM IR before handing over to codegen.
#if ENABLE_CUDA
