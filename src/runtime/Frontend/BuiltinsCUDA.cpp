...
Var<unsigned int> getGridDimX(FuncBase &Fn) {
  ...
  auto Call = Fn.getCodeBuilder().createCall(
      "llvm.nvvm.read.ptx.sreg.nctaid.x", TypeMap<unsigned int>::get());
  Ret.storeValue(Call);
...
Var<unsigned int> getGridDimY(FuncBase &Fn) {
  ...
  auto Call = Fn.getCodeBuilder().createCall(
      "llvm.nvvm.read.ptx.sreg.nctaid.y", TypeMap<unsigned int>::get());
  Ret.storeValue(Call);
...
Var<unsigned int> getGridDimZ(FuncBase &Fn) {
  ...
  auto Call = Fn.getCodeBuilder().createCall(
      "llvm.nvvm.read.ptx.sreg.nctaid.z", TypeMap<unsigned int>::get());
  Ret.storeValue(Call);
...
void syncThreads(FuncBase &Fn) {
  Fn.getCodeBuilder().createCall("llvm.nvvm.barrier0", TypeMap<void>::get());
}
