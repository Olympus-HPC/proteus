...
_PACKAGE = "proteus_backend_rocm"
_BACKEND_KIND = "rocm"
_BACKEND_PRIORITY = 20
...
def active_variant_spec() -> dict[str, object]:
    ...
    if explicit_variant and explicit_variant != str(variant["id"]):
        raise ImportError(
            f"Requested Proteus ROCm backend variant {explicit_variant!r} is not available. "
...
def is_runtime_compatible() -> bool:
    for env_name in ("PROTEUS_ROCM_PATH", "ROCM_PATH"):
